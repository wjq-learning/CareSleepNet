...
# print(psg_label_f_pairs[:39])

# for item in psg_label_f_pairs[:39]:
#     print(item)
...
print(label2id)
# %%
for psg_f_name, label_f_name in tqdm(psg_label_f_pairs[:39]):
    epochs_list = []
    labels_list = []

    raw = read_raw_edf(os.path.join(dir_path, psg_f_name), preload=True)
    # raw.filter(None, 49.9, fir_design='firwin')
...
# # print(epochs_array.shape, labels_array.shape)
# # # %%
# # index = epochs_array.shape[0]
# # while index % 20 != 0:
# #     index -= 1
# # epochs_array_ = epochs_array[:index, :, :]
# # labels_array_ = labels_array[:index]
# print(epochs_array_.shape, labels_array_.shape)
# epochs_seq = epochs_array_.reshape(-1, 20, 7, 3000)
# labels_seq = labels_array_.reshape(-1, 20)
# print(epochs_seq.shape, labels_seq.shape)
# print(epochs_seq.dtype, labels_seq.dtype)
#
# with open(r'/data/datasets/sleep-edf-independent/all_seqs.npy', 'wb') as f:
#     np.save(f, epochs_seq)
# with open(r'/data/datasets/sleep-edf-independent/all_labels.npy', 'wb') as f:
